...
```{r}
dat_full = dat_full %>% select(c(age, niveau, sexe, statmar, adulthab, travp, scolp, travm, scolm, negp1, negp8, negp9, negm1, negm8, negm9, dets1, mus2, mushr, parimp, muscol, musagg, aprmal, poster, pers, info, alien1, alien2, sui1, drog1, drog2, drog3, drog4))
```
...
dat_lm = dat_lm %>% select(-c("mus2_4", "mus2_5", "mus2_3", "mus2_2"))
...
PARTITION = sample(rep(1:5, rep(42,5)),210)
```
...
```{r}
# Régression normale
glm0 <- glm(sui1 ~ ., data=dat_lm, family = "binomial")
summary(glm0)
```
...
cv.glmn1 <- cv.glmnet(x= train.dat[,var.model] %>% as.matrix,
                      y = train.dat[,"sui1"], alpha = 0, nfolds = 5, 
...
cv.glmn2 <- cv.glmnet(x = train.dat[,var.model] %>% as.matrix,
                    y = train.dat[,"sui1"], alpha = 1, nfolds = 5,
...
for(al in seq(0.1,0.9,0.1)){
    ...
    y = train.dat[,"sui1"], nfolds = 5, foldid = PARTITION,
...
-----------SGL-------------
```{r}
cv_fit <- cv.sparsegl(x = train.dat[,var.model] %>% as.matrix, y = train.dat[,"sui1"], groups)
```
...
Pour finir, on calcule l'écart-type de la performance prédictive du modèle sur 1000 itérations à l'aide de bootstrapping.
...
good.class <- function(model, i ){
            ...
            glm1tp <- predict(glm1, newdata = test.dat[i,])
            cvt1 <- table(1*(glm1tp>0), test.dat$sui1[i])
            sum(diag(prop.table(cvt1)))*100
}
  else {glmn3tp <- predict(model,
                           newx = test.dat[i,var.model] %>% as.matrix, s = "lambda.min")
  (cvt4 <- table(1*(glmn3tp>0), test.dat$sui1)) %>%
...
#régression "normale"
sd(replicate(1000,good.class(glm1, sample(1:94, 94, TRUE))))
#régression avec régularisation ridge
sd(replicate(1000,good.class(cv.glmn1, sample(1:94, 94, TRUE))))
#régression avec régularisation lasso
sd(replicate(1000,good.class(cv.glmn2, sample(1:94, 94, TRUE))))
#régression avec régularisation elastic-net
sd(replicate(1000,good.class(cv.glmn3[[6]], sample(1:94, 94, TRUE))))
```

```{r output='hide', results='hide'}
...
cf <- data.frame(VAR = c("Int.", var.model),
                 ...
                 `ELASTIC NET` = drop(coef(cv.glmn3[[6]],
                                           s = "lambda.min",allCoef = TRUE)))
knitr::kable(cf, digits = 2,row.names = FALSE)
```